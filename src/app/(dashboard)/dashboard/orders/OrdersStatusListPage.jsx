...
export default function OrdersStatusListPage({
  ...
  const [quickFilters, setQuickFilters] = useState(() => {
    ...
    const next = { ...createDefaultToolbarFilters(), ...initialFiltersProp };
    if (affilated && initialFiltersProp?.affiliated_sku && !next.sku) {
      next.sku = initialFiltersProp.affiliated_sku;
    }
    return next;
  });
  ...
  const normalizeProductList = useCallback((response) => {
  ...
  const defaultProductListFetcher = useCallback(async () => {
  ...
  useEffect(() => {
    ...
    const loadVariations = async () => {
      try {
        const fetcher = productListFetcher || defaultProductListFetcher;
        const resp = await fetcher();
        if (!active) return;
        setProductVariations(normalizeProductList(resp));
      } catch (error) {
        if (!active) return;
        message.error(
      ...
      makeListRequest(
        listApiFn,
        {
          ...
          filterTransform: (filters = {}) => {
            ...
            if (affilated && next.sku) {
              next.affiliated_sku = next.sku;
              delete next.sku;
            }
  ...
  }, [applyFilterPatch, quickFilters, skuFilterKey]);
  ...
  const handleToolbarReset = useCallback(() => {
    ...
    const nextFilters = {
      ...resetState,
      [skuFilterKey]: resetState.sku,
    };
    if (affilated) {
      delete nextFilters.sku;
    }
    applyFilterPatch(nextFilters);
  }, [affilated, applyFilterPatch, skuFilterKey]);
    ...
    const baseColumns = [
      {
        ...
        width: 5,
        render: (_, record) => {
          const parentOrderQuantity = record?.children?.length + 1 || null;
        ...
        ? [
            {
              ...
              render: (_, record) => {
                ...
                const detailHref = `/dashboard/order/detail/${orderNumber}`;
                return (
                  <Tooltip title={t("actions.viewDetail")}>
                    <Button icon={<FileSearchOutlined />} href={detailHref} />
                  </Tooltip>
                );
              },
            },
