...
import ProductSizeForm from "@/components/common/forms/ProductSizeForm";
import ImportResultModal from "@/components/common/Modal/ImportResultModal";
import { ProductSizesAPI } from "@/utils/api";
...
export default function ProductSizesPage() {
  ...
  const t = useTranslations("dashboard.productSize");
  ...
  const onSubmit = async (values) => {
    try {
      if (editingRow) {
        await ProductSizesAPI.update(editingRow.id, values);
  ...
  const onDelete = async (id) => {
    try {
      await ProductSizesAPI.remove(id);
  ...
  const handleImportFileChange = async (event) => {
    ...
    setImporting(true);
    try {
      const response = await ProductSizesAPI.import(file);
