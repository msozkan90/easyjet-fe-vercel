...
export default function TransferOrderPoolListPage({ mode = "pending" }) {
  ...
  const cancelOrders = useCallback(
    async (records, options = {}) => {
      ...
      try {
        await Promise.all(ids.map((id) => OrdersAPI.transferPreCancel(id)));
      ...
      },
      {
        ...
        render: (_, record) => {
          ...
          const normalizedProductId = hasOwn.call(overrides || {}, "productId")
            ...
            : getNormalizedRecordValue(record, "product");
          const loading =
            productsLoading || isCellLoading(record.id, "product");
          return (
            <Select
              showSearch
              allowClear
            ...
            <Space size={[4, 4]} wrap>
              {linkedOrders.map((order) => (
                ...
                >
                  ...
                  {order?.order_status
                    ? tOrders(`status.values.${order.order_status}`) ||
                      order.order_status
                    : t("common.none")}
                  )
                </Button>
